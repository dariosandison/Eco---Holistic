...
export default function OfferExplorer({ offers }) {
  ...
  return <section><label className="text-sm font-semibold" htmlFor="offer-category">Filter offers</label><select id="offer-category" value={category} onChange={(event) => setCategory(event.target.value)} className="ml-3 rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm">{categories.map((item) => <option key={item}>{item}</option>)}</select><div className="mt-6 grid gap-4 md:grid-cols-2">{visible.map((offer) => <VisibilityTracker key={offer.id} event="offer_impression" data={{ offer_id: offer.id, merchant: offer.advertiser, category: offer.category }}><article className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500">{offer.category} · {offer.advertiser}</p><h2 className="mt-2 text-xl font-semibold">{offer.title}</h2><p className="mt-2 text-sm leading-6 text-zinc-700">{offer.description}</p><a href={offer.href} target="_blank" rel="sponsored noreferrer" className="btn-primary mt-5" data-affiliate-context={`offer:${offer.id}`}>View verified offer</a><p className="mt-4 text-xs text-zinc-500">{offer.terms} Ends {new Date(offer.endsAt).toLocaleDateString('en-GB')}.</p></article></VisibilityTracker>)}</div></section>
}
