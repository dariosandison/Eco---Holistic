...
export async function getStaticPaths() {
  return { paths: listSlugs('compare').map((slug) => ({ params: { slug } })), fallback: false };
}
    ...
    <>
      ...
      <div className="container">
        <article className="post">
          ...
          {items.length > 0 && (
            <>
              ...
              <div className="cmp-grid">
                {items.map((it, i) => {
                  const href = it.linkSlug ? `/go/${encodeURIComponent(it.linkSlug)}` : it.link || '#';
                  return (
                    <div className="cmp-card" key={i}>
                      ...
                      <div className="cmp-actions">
                        {href !== '#' ? (
                          <a className="btn btn-primary" href={href} rel="nofollow sponsored noopener noreferrer">
                            Check price
                          </a>
                        ) : null}
                      </div>
          ...
          {mdxSource ? <MDXRemote {...mdxSource} components={{ Callout, CompareInline }} /> : null}
