// lib/content.js
// Server-only content utilities (uses Node 'fs').
// Ensures all dates are returned as strings so Next.js can serialize props.
...
// ---- Date normalization (critical for Next serialization) ------------------
function toDateString(v) {
  // Convert Date to ISO string; leave strings as-is; ignore others
  if (v instanceof Date) return v.toISOString(); // full ISO so TZ is unambiguous
  return typeof v === 'string' ? v : undefined;
}
function normalizeFrontmatter(data) {
  const out = { ...data };
  if (data?.date !== undefined) {
    const s = toDateString(data.date);
    if (s) out.date = s;
...
export function getAllDocs({ dir, fields = [] }) {
  ...
  // Sort by date desc if present (dates are strings now)
  items.sort((a, b) => {
...
export function getDocBySlug({ dir, slug, fields = [] }) {
  ...
  const out = { slug };
  fields.forEach(f => { if (f in fm) out[f] = fm[f]; });
  out.content = content;
  return out;
}
